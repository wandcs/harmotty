...
以下方向用于以后评估，不自动建立活动开发任务：

### 8.1 设备控制通道预检

在现有 HDC helper 上提供一个快速、无产品副作用的预检入口，集中报告设备连接、锁屏、
PID、layout、hilog、终端焦点和受控服务器状态。预检必须在一分钟级完成，并明确失败域。
...
上述 helper 和阶段续跑能力都不是当前活动任务，也不能替代现有手工或脚本门禁。只有
`next-work.md` 明确授权后才实施；实施完成前，当前工作继续使用第 7 节列出的权威入口。
