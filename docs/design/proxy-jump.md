...
- 优先读取 OpenSSH config 的 `ProxyJump`；是否加入一次性 `-J` 仍待用户场景验证。
...
- `ProxyJump jump-alias` 必须通过与目标相同的 OpenSSH config 解析器解析。
...
是否提供 `ssh -J jump target` 取决于它能否保持“一次性覆盖而非第二套配置”的现有
命令模型；在确认前不加入路线图具体功能点。
...
必须引入第二套 UI/状态机，应停止 1.5，而不是退化为远端 shell 拼接命令或扩张成网络
代理平台。
