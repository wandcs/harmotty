...
1. 完成 1.1 发布核心 SSH `keyboard-interactive` 与多方法认证。
2. 然后实施可独立裁剪的 Tab 导航；Pane 导航必须先通过真机前置门禁。
3. 最后完成公共检查、干净 ARM64 构建和物理 HarmonyOS PC 集成矩阵。
4. SSH 认证核心门禁未通过时，不进入 1.1.0 发布准备。导航失败可以裁剪，
   不能用新增设置、别名或特殊分支补救。
