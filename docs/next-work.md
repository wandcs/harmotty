...
1. 先完成已经确认归属 1.1 的 SSH 命令基线：密钥口令修改、known-hosts 查询、未知
   option/config 的明确失败和现有命令 help/test 边界；非 1.1 能力只按路线图分配，
   不提前实现。
2. 再闭合已经实现但缺少最终证据的 1.0/1.1 可靠性门禁，修复会污染验收结论的测试
   工具。
3. 再完成 1.1 发布核心 SSH `keyboard-interactive` 与多方法认证。
4. 然后实施可独立裁剪的 Tab 导航；Pane 导航必须先通过真机前置门禁。
5. 最后完成公共检查、干净 ARM64 构建和物理 HarmonyOS PC 集成矩阵。
6. 持久资产或 SSH 认证核心门禁未通过时，不进入 1.1.0 发布准备。导航失败可以裁剪，
   不能用新增设置、别名或特殊分支补救。

## 0. SSH 命令体系 1.1 基线

正式能力矩阵、明确不做范围和后续 milestone 分配：
...
- [ ] 建立 SSH CLI/config 的明确不支持边界：未知 option、互斥/缺失参数，以及未知或会
  影响目标、认证、主机信任、网络路径和 Session 的未支持 directive，必须在连接或
  资产变更前给出可操作错误；不得静默忽略后声称兼容。
- [ ] 逐项核对当前 `ssh`、`ssh-keygen`、`ssh-copy-id`、`host`、`key` 的 parser、help、
  补全和自动化，与 Accepted 矩阵一致；未支持的高风险/易误判语法明确报错，历史别名
  不扩展为新能力。
